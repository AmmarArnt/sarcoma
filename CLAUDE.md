...
- `simulation-output/vaccine-int-concept-transplant-layer.md` — **M5 (vaccine) deep-dive**: takes the *concept*
  behind individualized neoantigen therapy (Merck/Moderna's intismeran autogene / mRNA-4157) rather than the drug,
  decomposes it into four pillars (**P1** private-mutanome antigen source · **P2** mRNA-LNP delivery+adjuvanticity ·
  **P3** polyepitope breadth · **P4** MRD-window + PD-1 deployment), and tests which transplant. Backed by
  `sims/11-vaccine-antigen-portfolio/` (ADR-0023). Headline: **only P1 is tumour-specific and it is the one that breaks** —
  mean 2.04 candidate epitopes and **P(fills a 34-slot construct)=0.002** vs 57.3/0.40 for the same funnel on melanoma
  (28×), surviving a **10× TMB stress test**. Second headline: **antigen count is not the binding constraint** — ablating
  the NK arm costs −89% and losing the MRD window −52%, while halving epitope breadth costs −3.4%; and **no vaccine
  architecture beat the no-vaccine baseline** for this case. Standing generalization — the **antigen-source inversion**:
  melanoma personalizes because its antigens are *private*; translocation-driven sarcoma's best antigens are *public*, so
  the right architecture is **off-the-shelf**, and the concept transplants cleanly to the **complex-karyotype** half of
  sarcoma (UPS/DDLPS/angiosarcoma) rather than this one. Reuse for vaccine / neoantigen / personalized-cancer-vaccine /
  "is drug X's concept applicable here" questions. **Not a fifth vector** — the M5 cell of ADR-0018 worked through V4.
  **All citations `[VERIFY]` (egress blocked 2026-08-19) → forward lane, gated out of protocol promotion by ADR-0020.**
...
- `sims/01–10/` — executed in-silico experiments with `RESULTS.md` + data `MANIFEST.md` + grounding.
  ...
  decision model above; Sim 10 = the **chemo-response cell-state** update extending Sim 8; Sim 11 = the **vaccine
  antigen-supply / architecture** model behind the M5 concept-transplant layer.)
...
| A **vaccine / neoantigen** question — cancer vaccines, personalized or off-the-shelf, mRNA constructs, "could a vaccine work here," **or a "is drug/platform X's *concept* applicable to this tumour" question of any kind** | **Reuse the M5 vaccine concept-transplant layer** (`simulation-output/vaccine-int-concept-transplant-layer.md` + `sims/11-vaccine-antigen-portfolio/` / ADR-0023). Method to copy for any "is concept X applicable" ask: **decompose the concept into pillars and find which one is tumour-specific**, rather than surveying the drug. For vaccines that pillar is the **antigen supply**, and it fails in translocation-driven sarcoma — apply the **antigen-source inversion** (private → public antigens; keep delivery/breadth/deployment). Report the **design** answer and the **this-case** answer separately; they differ. Two standing method requirements from ADR-0023: a **calibration arm** (reproduce the indication where the concept was validated) and a **null baseline** (no-intervention arm). **Not a fifth vector**; junction/DUX4-based payloads are **fusion-contingent** (golden rule #9). |
...
sims/01–11/              reproducible simulations (script + RESULTS.md + MANIFEST.md + grounding.tsv)
simulation-output/       protocol-v4.md (main catalog; v1–v3 baselines), findings-ranking.md (master register),
                         ...
                         chemosensitivity-ddr-cellstate-layer.md, vaccine-int-concept-transplant-layer.md,
                         tumorigenesis-reverse-engineering/,
                         per-vector outputs
```
