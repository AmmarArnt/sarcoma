...
| 9 | Condensate / LLPS of the DUX4 transactivation module (FORWARD Track B) | UniProt Q9UBX2/Q01844/P35637/Q96RK0 (CRC64-verified fetch) | localcider Das-Pappu/Uversky descriptors (+optional metapredict) | **Toolchain validated; biological run network-blocked** in this environment (UniProt egress-403 → aborts rather than fabricate). Delivers a runnable, integrity-checked pipeline + **Forward Hypothesis FH-9.1**: CIC-DUX4 nucleates a **p300/CBP-dependent acetylation-driven transcriptional condensate** via the junction-invariant DUX4 C-term IDR (fusion-agnostic → covers the ~5% unconfirmed case); falsifier = p300/CBP-inhibitor (A-485/inobrodib) condensate-dissolution test. No DUX4/CIC-DUX4 LLPS study exists (2026-06 sweep). |
...
- **Sim 9 condensate/LLPS — execute for real** in a network-permissive environment (the pipeline is committed and CRC64-gated; only the UniProt fetch was egress-blocked here). Then add FuzDrop/PScore/catGRANULE web-server scores.
