...
9. **Scoring axes beyond tier, where they apply.** Tier is the first of three orthogonal axes (see `/sarcoma-contract`). Add a **confidence** read (Directness / in-vivo Achievability / Reproducibility / conflict — `docs/08-evidence-confidence-scoring.md`) where a bare tier could mislead, and a **translational feasibility band** (F1 Accessible-now … F5 Concept-only — `simulation-output/translational-feasibility-layer.md`) on any clinical/experimental entry whose real-world access matters. Two-lane rule: these annotate the confirmatory lane, never prune the Forward-Hypotheses lane.
...
9. **Stale regulatory / trial / feasibility status.** "Tazemetostat is FDA-approved for epithelioid sarcoma" was true in 2020 and false after the 2026-03-09 US market withdrawal. Any "approved / recruiting / discontinued / on-hold / withdrawn" claim must be **verified live this session** against the registries in `docs/09-verification-sources.md` (ClinicalTrials.gov, EU CTIS, Drugs@FDA, EMA, PMDA), with source + access date recorded and the right jurisdiction checked (FDA ≠ EMA ≠ PMDA). A status carried over unchecked is a fabrication risk — tag `[VERIFY]` if you cannot confirm. (See the three scoring axes and the perishable-status citation rule in `/sarcoma-contract`; ADR-0004.)
