...
| **M5** vaccines | **Deep-dived** — [`vaccine-int-concept-transplant-layer.md`](vaccine-int-concept-transplant-layer.md) (ADR-0023), on top of the neoantigen-vaccine + mRNA team outputs | covered **and worked through** (fusion-contingent for junction vaccines) | The INT/intismeran **concept** decomposes into 4 pillars; **only the private-mutanome antigen supply is tumour-specific, and it fails here** (2.04 candidate epitopes; P(fills 34 slots)=0.002 vs 0.40 in melanoma). **Antigen-source inversion:** translocation-driven sarcoma's best antigens are *public*, so the right architecture is off-the-shelf; the concept transplants to the **complex-karyotype** half of sarcoma instead. Antigen count is **not** the binding constraint (breadth −3.4% vs window −52%, NK arm −89%) |
