...
> **Sim 9** condensate result. Baselines `protocol-v1.md` / `v2.md` / `v3.md` are **preserved** (CLAUDE.md §0;
> the standing live-evidence-refresh lane is **ADR-0020**). **What changed vs v3, in one line:** the
> immune/MHC-I strategy gained a verified, CIC-DUX4-*specific*, clinical-stage MHC-I-restoring node —
> **p300/CBP** — that consolidates V1/V3 throttle + fusion-destabilisation + V4 priming and replaces the
> withdrawn EZH2i bridge; the cell-cycle/throttle backbone is unchanged; one v3 supporting claim (a WEE1
> "validation") was **retracted on verification** (snippet-sourcing error — see ADR-0020 / refresh §A).
...
3a. **[NEW in v4] p300/CBP is a single node that hits THREE vectors at once — and a clinical-stage
   inhibitor exists — making it the most direct, CIC-DUX4-specific, still-accessible MHC-I bridge.**
   The same coactivator that (a) is *required* for CIC-DUX4 transactivation [V1/V3 throttle] and (b)
   maintains fusion-protein stability via acetylation [destabilised on inhibition] also (c) **suppresses
   MHC-I — and its inhibition restores MHC-I, converting the cold tumour "hot" and driving regression in
   an immunocompetent CIC::DUX4 model** [V4 priming]. This is the verified 2025 delta that re-anchors the
   MHC-I bridge away from EZH2i (off-table, #2/#3) onto p300/CBP. **Clinical-stage (verified 2026-06-25,
   perishable):** inobrodib/CCS1477 (NCT04068597 RECRUITING heme; NCT03568656 COMPLETED solid) and
   TT125-802 (NCT06403436 ACTIVE-NOT-RECRUITING solid) — **no sarcoma arm yet; p300/CBP is broadly
   essential, so therapeutic-window/on-target toxicity is the real risk.** [Preclinical-Cell/Animal,
   CIC-DUX4-direct — Mol Cancer 2025 PMID 41299516 (MHC-I); Oncogenesis 2021 PMID 34642317 + Cancers 2024
   PMID 38275898 (requirement); Medium-High confidence — 3 concordant papers, **NOT Established**] **F3.**
   *Fusion-agnostic.*
...
4. **The highest-ceiling NOVEL target — MCL1 ("re-arm the DUX4 death program") — is DRIVER-CONTINGENT and
   ...
   / PMC12370961; PMID 40841360 / PMC12371069 — both full-text-verified 2026-06-25) show CIC::DUX4 tumoroids
   depend on MCL1 because the retained DUX4 transactivation domain forces an anti-apoptotic buffer; MCL1 is a
   *direct transcriptional target* of the fusion. That domain exists only if the driver is truly CIC-DUX4 (D1).
   **[NEW in v4 — patient-specific cardio-gate]** the MCL1i class carries a documented cardiotoxicity signal
   (ABBV-467 troponin in 4/8 patients PMID 37880389; AZD5991 cardiac-arrest AE PMID 39167622; next-gen BRD-810
   re-engineered for lower cardiac exposure PMID 39179926) that **stacks on this patient's prior doxorubicin**
   → feasibility is **F3–F4 and cardio-gated**, not a clean novel target (an oncologist owns any cardiac-risk
   judgement). [Preclinical-Cell + Preclinical-Animal, **independently replicated** — highest-tier *direct*
   CIC-DUX4 finding in the catalog] **Fusion-confirmed only — HOLD until driver resolved.**
...
8. **Checkpoint-inhibitor monotherapy carries a NEGATIVE-leaning prior here:** sarcoma CPI response (SARC028,
   ...
   genomically-simple, presumed-low-TMB profile (Italiano, PMID 27664537, CIC-DUX4-direct). The March-2025
   case report (**PMID 40128305, full-verified 2026-06-25** — first reported ICB response in CDS) shows a
   *fusion-confirmed* CIC::DUX4 tumour converting cold→hot under **nivolumab+relatlimab (PD-1 + LAG-3)**,
   with pre-tx immune-cold histology and post-tx CD8 influx + exhaustion-marker co-expression → supports a
   **priming-then-checkpoint, PD-1+LAG-3 doublet** sequencing read (consistent with Sims 4/5), a disease-class
   precedent only (n=1). [Clinical, single case] *Fusion-agnostic mechanism.*

8a. **[NEW in v4] DUX4 is an intrinsic interferon antagonist — a second, MHC-I-independent immune-evasion
   arm.** DUX4 binds STAT1 and broadly suppresses IFN-γ-stimulated genes — including **blocking IFN-γ-induction
   of MHC-I** — via conserved (L)LxxL(L) motifs in the **junction-retained DUX4 C-terminus** (eLife 2023,
   PMID 37092726, peer-reviewed). Implication: simply *adding* IFN is blunted at the source; the lever is to
   **bypass IFN** with direct MHC-I restoration (p300/CBPi #3a, or HDACi/DNMTi). [Preclinical-Cell,
   peer-reviewed] *Fusion-relevant (DUX4 moiety → applies to the ~5% via the conserved C-terminus).*
...
| **dCBP-1 (p300/CBP degrader)** | V3 | CIC-DUX4 needs p300/CBP for H3K27ac at target loci; degrading p300/CBP silences fusion output — **the one degrader entry with DIRECT CIC-DUX4 cell-line data** (PMC8511258) | Preclinical-Cell | n/a (preclinical) | n/a | — | **F5 / R0 never-built / P0 (direct)** | **Partially driver-contingent** (strongest D1/D2). Technology watch-item; the clinical-stage route to the *same node* is the p300/CBP bromodomain-inhibitor row below. |
| **p300/CBP bromodomain inhibitors** (inobrodib/CCS1477; TT125-802) **[NEW in v4]** | **V1, V3, V4 (MHC-I)** | p300/CBP is *required* for CIC-DUX4 H3-acetylation/transactivation; inhibition reverses fusion acetylation, **restores MHC-I (cold→hot, regression in an immunocompetent CIC::DUX4 model)**, and destabilises the fusion — the **single multi-vector node** | Preclinical-Cell/Animal (CIC-DUX4-direct: PMID 34642317, 38275898, **41299516**) | Investigational | Investigational | inobrodib **NCT04068597 RECRUITING** (heme) / **NCT03568656 COMPLETED** (solid); TT125-802 **NCT06403436 ACTIVE-NOT-RECRUITING** (solid) — *all verified 2026-06-25* | **F3 / R-n/a / P0 (direct)** | **Fusion-agnostic. The consolidated multi-vector node + the most direct still-accessible MHC-I bridge** (replaces EZH2i). No sarcoma arm; broad-essentiality → therapeutic-window/on-target toxicity is the real risk. Status perishable — re-verify. |
| **EZH2/EZH1 inhibitors** (tazemetostat; valemetostat) | V3→V4 | PRC2/H3K27me3 removal restores APM in PRC2-dependent tumours | Doubly caveated | **Tazemetostat WITHDRAWN WORLDWIDE 2026-03-09** (epithelioid sarcoma 2020-01-23 + FL); **never EMA-approved** | Never EMA-approved | Valemetostat NCT07303387 (SWI/SNF-altered); FDA-approved 2022 ATLL (heme) | **Tazemetostat F5 / R4 regulatory-safety (NOT R1) ; valemetostat F2 / P3** | **Fusion-agnostic mechanism, but premise contested (p300/CBP, not PRC2 — Bakaric 2024) AND access closed.** The MHC-I bridge is now preferentially anchored on **p300/CBPi** (CIC-DUX4-specific MHC-I restoration, PMID 41299516 — see the p300/CBP bromodomain-inhibitor row), with **HDACi/DNMTi as the FDA-approved-repurposable fallback.** Valemetostat inherits an elevated secondary-malignancy index-of-suspicion (broader PRC2 blockade) — open safety question, not reassurance. **Do NOT present as an actionable bridge.** |
...
**FH-8 (condensate; = FH-9.1 in Sim 9 — EXECUTED 2026-06-25). CIC-DUX4 organises a transcriptional condensate
*heterotypically* — its acidic DUX4 C-term partitioning into a p300/CBP coactivator hub — NOT via FET-type
prion-like self-assembly.** *Statement & what changed:* the naive transfer from the EWSR1/FET-fusion condensate
literature was **tested in-silico and refuted** — Sim 9 (localcider + metapredict + PLAAC) shows the DUX4 C-term
is a disordered **acidic activation domain**, PLAAC prion-domain score **0** vs EWSR1 77.6 / FUS 113.7, clustering
with the folded CIC HMG-box control. *Mechanistic basis:* an acidic AD drives condensation by partitioning into
coactivator (p300/CBP) condensates, not by self-association — exactly the DUX4 C-term's defined "interaction with
EP300/CREBBP" function, linking this to the **same p300/CBP node as #3a**. *Falsifier (named):* CIC-DUX4 puncta
should be **p300/CBP-inhibitor (A-485/inobrodib)- and 1,6-hexanediol-sensitive**; if they self-assemble
independent of coactivators the model is wrong. *Fusion-agnostic* (every junction keeps the DUX4 C-terminus →
covers the ~5%). [**Theoretical** — predictors are out-of-distribution for an artificial fusion module; tier ceiling
holds. See `sims/09-condensate-llps/RESULTS.md`.] **Heterotypic-aware web-server scores (FuzDrop/PScore/catGRANULE)
still pending — servers unreachable 2026-06-25.**
...
regulatory/trial facts: **2026-06-14** unless noted; **v4 additions full-text/abstract-verified 2026-06-25.**

**[NEW in v4 — verified 2026-06-25] CIC-DUX4 immune evasion + targets:**
- **PMID 41299516**, *Mol Cancer* 2025;24(1):299, PMC12659477, DOI 10.1186/s12943-025-02485-6 — CIC::DUX4/p300/CBP-mediated **MHC-I suppression**; p300/CBP inhibition restores MHC-I (cold→hot, regression). *(load-bearing for v4 #3a)*
- **PMID 34642317**, *Oncogenesis* 2021;10(10):68, PMC8511258, DOI 10.1038/s41389-021-00357-4 — CIC-DUX4 *requires* p300/CBP; iP300w/A-485 reverse acetylation + block CDS xenografts.
- **PMID 37092726**, *eLife* 2023;12:e82057, DOI 10.7554/eLife.82057 — DUX4–STAT1 interaction broadly inhibits ISGs / blocks IFN-γ-MHC-I induction *(peer-reviewed; supersedes bioRxiv 2022.08.09.503314)*.
- **PMID 34903601**, *Cancer Res* 2022, DOI 10.1158/0008-5472.CAN-21-1222 — HMGA2/IGF2BP/IGF2/IGF1R/AKT-mTOR axis → trabectedin + PI3K/mTOR sensitivity in CDS PDX models.
- MCL1i class cardiotoxicity: **PMID 37880389** (ABBV-467 troponin, *Commun Med* 2023), **PMID 39167622**/PMC11528199 (AZD5991 ph1, *Clin Cancer Res* 2024), **PMID 39179926** (BRD-810 next-gen, *Nat Cancer* 2024).
- p300/CBP bromodomain-inhibitor trials (verified 2026-06-25, perishable): inobrodib/CCS1477 NCT04068597 (RECRUITING), NCT03568656 (COMPLETED); TT125-802 NCT06403436 (ACTIVE-NOT-RECRUITING).

**CIC-DUX4 biology / targets:**
- Bakaric et al., *Cancers* 2024;16(2):457, PMID 38275898, DOI 10.3390/cancers16020457 — CIC-DUX4 p300/CBP activator (dataset GSE248040, verified 2026-06-25).
...
- Johnson et al. *Nat Commun* 2021, PMID 34376667 (PEMDAC). CIC::DUX4 dual-ICB case (nivolumab+relatlimab) *npj Precis Oncol* 2025;9(1):85, **PMID 40128305** — verified 2026-06-25.
